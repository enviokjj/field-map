...
**★기본은 온라인이다 — 오프라인 대비가 온라인을 망치면 안 된다.** 두 가지를 지킨다:
...
- **캐시를 둘로 나눈다.** 앱(약 5.6MB)은 내용이 바뀌면 판이 올라가 **항상 최신**이 되고,
  배경지도 79MB 는 목록이 바뀔 때만 판이 올라간다. 한 판으로 묶으면 `index.html` 을
  한 줄 고칠 때마다 79MB 를 다시 받게 된다.
